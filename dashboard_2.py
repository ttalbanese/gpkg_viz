...
SUBTITLE = (
    "Upload multiple GeoPackages, select tables & common columns, and export a publication-ready scatter plot."
)
...
# ---- Widgets -------------
# -------------------------
file_input = pn.widgets.FileInput(name="Upload GeoPackages", accept=".gpkg", multiple=True, width=320)
clear_btn = pn.widgets.Button(name="Clear Uploads", button_type="warning", icon="trash", width=160)

tables_accordion = pn.Accordion(name="Tables by GeoPackage", active=[], sizing_mode="stretch_width")
...
alpha_slider = pn.widgets.FloatSlider(name="Point alpha", start=0.2, end=1.0, step=0.05, value=0.7, width=280)
size_slider = pn.widgets.IntSlider(name="Point size", start=3, end=14, step=1, value=7, width=280)
legend_toggle = pn.widgets.Checkbox(name="Show legend", value=True)
grid_toggle = pn.widgets.Checkbox(name="Show grid", value=True)
title_input = pn.widgets.TextInput(name="Plot title", placeholder="Optional plot title", width=320)
...
export_dpi = pn.widgets.IntSlider(name="DPI (for PNG)", start=100, end=600, step=50, value=300, width=300)
...
status_alert = pn.pane.Alert('', alert_type='info', visible=False, sizing_mode='stretch_width')

# -------------------------
# ---- Data State ----------
# -------------------------
TMPDIR = tempfile.TemporaryDirectory(prefix="gpkg_compare_")
GPKG_DATA = OrderedDict()        # {gpkg_name: {"path": path, "layers": {layer_name: DataFrame}}}
...
def numeric_common_columns(selected_tables):
    ...
    return sorted(set.intersection(*numeric_sets) if numeric_sets else set(), key=lambda x: x.lower())
...
def update_column_selects(event=None):
    ...
    def _set_values():
        if len(commons)>0:
            ...
            x_select.value = None
            y_select.value = None
            status_alert.object = "No common numeric columns across the selected tables."
...
def build_plot_df(selected_tables, x_col, y_col):
    ...
    for (gpkg_name, layer), df in selected_tables.items():
        ...
        if x_col in df.columns and y_col in df.columns:
            ...
            rows.append(sub)
    return pd.concat(rows, axis=0, ignore_index=True) if rows else pd.DataFrame(columns=[x_col, y_col, "group"])
...
def _empty_plot_frame(grid, title):
    ...
    txt = hv.Text(0.5, 0.5, "Select tables and X/Y…").opts(text_align='center', text_baseline='middle', fontsize=12)
    return (base * txt).opts(hv.opts.Overlay(title=title or "Scatter Plot", bgcolor='white'))
...
def make_hv_scatter(x, y, alpha, size, legend, grid, title, trigger):
    ...
    for g in groups:
        ...
        ).opts(muted_alpha=0.1).relabel(g)
        overlays.append(sc)

    # Update point count diagnostics
    points_count_md.object = f"##### Points plotted: {len(df):,} across {len(groups)} group(s)"
...
def export_figure_callback():
    ...
    if not xcol or not ycol:
        raise pn.io.FileDownloadCallbackError("Nothing to export. Please select tables and X/Y columns.")
    xcol = str(xcol); ycol = str(ycol)

    df = build_plot_df(selected, xcol, ycol)
    if df.empty:
        raise pn.io.FileDownloadCallbackError("No data available for the selected X/Y across chosen tables.")
    ...
    for g in groups:
        ...
        ax.scatter(
            ...
            c=color_map[g], edgecolors='none', label=g
        )
    ...
    if legend_toggle.value:
        ax.legend(loc='center left', bbox_to_anchor=(1.02, 0.5), frameon=False, fontsize=10, title="Group")
    ...
    fig.tight_layout()
    fig.savefig(buf, format=fmt, dpi=dpi, bbox_inches='tight', facecolor='white', edgecolor='white')
    plt.close(fig)
    buf.seek(0)
    return buf.getvalue()
...
def rebuild_tables_accordion():
    ...
    for gpkg_name, meta in GPKG_DATA.items():
        ...
        chk = pn.widgets.CheckBoxGroup(name=gpkg_name, options=layer_names, inline=False, width=300)
...
def _handle_uploaded_files(event=None):
    ...
    if isinstance(v, (list, tuple)):
        for data, name in zip(v, names): pairs.append((name, data))
    else:
    ...
    for name, data in pairs:
        ...
        while gpkg_key in GPKG_DATA:
            gpkg_key = f"{os.path.splitext(base)[0]}_{i}.gpkg"; i += 1
...
def _clear_uploads(event=None):
    ...
    status_alert.visible = True
    common_cols_md.object = ""; common_cols_md.visible = False
    points_count_md.object = ""
...
sidebar = pn.Column(
    ...
    "#### Plot options",
    title_input,
    pn.Row(alpha_slider, size_slider),
    pn.Row(legend_toggle, grid_toggle),
    pn.layout.Divider(),
    "#### Export",
    export_format, export_dpi,
    export_btn,
